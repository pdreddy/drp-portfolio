    ...
    <section id="work" className="section editorial-section">
      <div className="shell">
        <SectionHeading
          ...
          title="Architecture and modernization across mission-critical enterprise systems."
        />
        <div className="impact-list">
          {caseStudies.map((item, index) => (
            <article className="impact-row reveal" key={item.title}>
              <span className="row-index">0{index + 1}</span>
              <div className="impact-title">
                <p>{item.sector}</p>
                <h3>{item.title}</h3>
              </div>
              <p className="impact-copy">{item.contribution}</p>
